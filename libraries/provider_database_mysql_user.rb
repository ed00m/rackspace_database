...
class Chef
  class Provider
    class RackspaceDatabase
      # a more specific provider for mysql users
      class MysqlUser < Chef::Provider::RackspaceDatabase::Mysql
        ...
        def action_grant
          ...
          grant_statement = "GRANT #{@new_resource.privileges.join(', ')} " \
            "ON #{@new_resource.database_name && @new_resource.database_name != '*' ? "`#{@new_resource.database_name}`" : '*'}" \
            ".#{@new_resource.table && @new_resource.table != '*' ? "`#{@new_resource.table}`" : '*'} TO `#{@new_resource.username}`@`#{@new_resource.host}` IDENTIFIED BY "
